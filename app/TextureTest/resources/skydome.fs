...
//the color of the sky on the half-sphere where the sun is. (time x height)
...
void main(){
    ...
    vec3 color_w_sun = texture(tint, vec2((iGlobalTime + 1.0) / 2.0,max(0.01,pos_norm.y))).rgb;
    vec3 color_wo_sun = color_w_sun;
    color = color_w_sun; //weather*mix(color_wo_sun,color_w_sun,dist*0.5+0.5);
