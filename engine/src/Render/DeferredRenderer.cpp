...
void DeferredRenderer::setupShadowMapUniforms(GLProgram * prog)
{
    ...
    for (int i = 0; i < NUM_SHADOWS; i++) {
        ...
        glm::vec4 shadowVec(0.0f, 0.0f, _shadowMaps.getShadowZ(i), 0.0f);
        shadowVec = _mainCamera->getProjection() * shadowVec;
        glUniform1f(prog->getUniformLocation(shadowZName), shadowVec.z);
    }
