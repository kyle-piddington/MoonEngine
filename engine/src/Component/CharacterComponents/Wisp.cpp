...
void Wisp::start()
{
	...
	pointLightComponent = gameObject->getComponent<PointLight>();
	if (player != nullptr)
	{
		playerBrightness = player->getComponent<PlayerBrightness>();
	}
...
void Wisp::update(float dt)
{
	...
	accumTime += dt;
	scale = 0.1f * (1 - accumTime);//1.0f / (8.0f * dt);//(2.0f - dt)/ 4.0f;
