...
bool verifyFile(const rapidjson::Document & document)
{
    ...
    if (!shaderOK)
    {
        LOG(ERROR, "Shader from program " + std::string(
                document["name"].GetString()) + " Could not be loaded, no vertex shader tag");
        return shaderOK;
    }
    ...
    {
        LOG(ERROR, "Shader from program " + std::string(
                document["name"].GetString()) + " Could not be loaded, no fragment shader tag");
        return shaderOK;
    }
