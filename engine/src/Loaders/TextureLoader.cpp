...
std::shared_ptr<GLTexture> TextureLoader::LoadTextureFromFile16f(GLuint unit, const std::string & textureName)
{
    int width = 0, height = 0, ncomps = 0;

    stbi_set_flip_vertically_on_load(true);
    unsigned short * data = stbi_load_16(textureName.c_str(), &width, &height, &ncomps, 0);
    /* File not found */
    if (!data)
    {
...
int TextureLoader::LoadTextureToBuffer16f(unsigned short ** bfr, const std::string & textureName, int * wd, int * ht)
{
    int width = 0, height = 0, ncomps = 0;

    stbi_set_flip_vertically_on_load(true);
    *bfr = stbi_load_16(textureName.c_str(), &width, &height, &ncomps, 0);
    /* File not found */
    if (!*bfr)
    {
        LOG(ERROR, "Could not load texture from file named " + textureName);
